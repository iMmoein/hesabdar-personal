...
export default function RevenuePage() {
  ...
  const formDirty = useMemo(
    () => Boolean(amount) || Boolean(accountId) || date !== todayJalaliString(),
    [amount, accountId, date]
  )

  const resetForm = () => { setAmount(''); setAccountId(''); setDate(todayJalaliString()); setEditingId(null) }
  ...
  const openEditForm = (rev) => {
    ...
    setDate(rev.date) // This is already a Jalali "YYYY/MM/DD" string
    setShowForm(true)
  }
  ...
  const handleSubmit = () => {
    ...
    else { addRevenue({ amount: Number(amount), accountId, date }); resetForm(); setShowForm(false) }
  }
  ...
  const confirmDelete = () => { if (deleteTarget) { deleteRevenue(deleteTarget.id); setDeleteTarget(null) } }

  return (
    <div className="px-4 pt-4 pb-28 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">درآمد</h1>
        <button onClick={openAddForm} className="btn-primary"><Plus size={20} /> ثبت درآمد</button>
      </div>
      ...
      <div className="space-y-2">
        {sorted.length === 0 && (
          <div className="card p-8 text-center text-slate-400"><TrendingUp size={40} className="mx-auto mb-2 opacity-40" /><p>هنوز درآمدی ثبت نشده است</p></div>
        )}
        {sorted.map((rev) => {
          ...
          return (
            <div key={rev.id} className="card p-3 flex items-center gap-3">
              ...
              <button onClick={() => openEditForm(rev)} className="p-2 rounded-lg text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-900/20 shrink-0"><Pencil size={16} /></button>
              <button onClick={() => setDeleteTarget(rev)} className="p-2 rounded-lg text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 shrink-0"><Trash2 size={16} /></button>
            </div>
          )
        })}
      </div>

      <Modal open={showForm} onClose={() => { setShowForm(false); resetForm() }} title={editingId ? 'ویرایش درآمد' : 'ثبت درآمد'} dirty={formDirty} onSave={handleSubmit} onDiscard={() => { setShowForm(false); resetForm() }}
      ...
      <ConfirmActionDialog open={editConfirm} onConfirm={confirmEdit} onCancel={() => setEditConfirm(false)} title="ویرایش درآمد" message="آیا از ویرایش این مورد اطمینان دارید؟" confirmLabel="ذخیره تغییرات" />
      <ConfirmActionDialog open={!!deleteTarget} onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} title="حذف درآمد" message="آیا از حذف این مورد اطمینان دارید؟" confirmLabel="حذف" confirmClass="btn-danger" />
    </div>
  )
}
