...
export default function App() {
  ...
  return (
    <div className="app-bg min-h-[100dvh] flex flex-col">
      <div className="flex-1 overflow-y-auto no-scrollbar"><ActiveComponent /></div>
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border-t border-slate-100 dark:border-slate-700 z-40" style={{ paddingBottom: 'var(--safe-bottom)' }}>
        <div className="flex items-center justify-around max-w-md mx-auto px-2">
          {TABS.map((tab) => {
            const Icon = tab.icon
            const isActive = activeTab === tab.id
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-col items-center gap-1 py-2.5 px-3 transition flex-1 ${isActive ? 'text-brand-600 dark:text-brand-400' : 'text-slate-400 dark:text-slate-500'}`}>
